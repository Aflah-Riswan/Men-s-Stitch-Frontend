...
const AddProducts = () => {
  ...
  function handleVariantImageUpload(e) {
    ...
    const updatedList = [...variantImages, ...newFiles]
    setVariantImages(updatedList)
    setValue('variantImages', updatedList, { shouldValidate: true })
    e.target.value = ''
  ...
  function handleCoverImageUpload(e) {
    const newFiles = Array.from(e.target.files)
    const totalFiles = newFiles.length + coverImages.length

    if (totalFiles > 3) {
      setError('coverImages', { type: 'manual', message: 'Maximum 3 images are alloweds' })
      return
    }
    clearErrors('coverImages')
    const updated = [...coverImages, ...newFiles]
        ...
        <form action="" onSubmit={handleSubmit(onSubmit)} >
          ...
          <input
            ...
            {...register("coverImages", {
              validate: (value) => (value && value.length === 3) || "Please upload exactly 3 COVER images."
            })}
          />
          <input
            type="hidden"
